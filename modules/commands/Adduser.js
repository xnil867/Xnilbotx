...
module.exports.run = async function ({ api, event, args }) {
	...
	if (!args[0]) return out("Please insert a UID\ninsert\nHow to use?\n\n/adduser <uid>\n\nExample\n/adduser\n\nCreated by: 𝗦𝗵𝗮𝗼𝗻 𝗔𝗵𝗺𝗲𝗱");
	if (!isNaN(args[0])) return adduser(args[0], undefined);
	else {
		try {
			var [id, name, fail] = await getUID(args[0], api);
			if (fail == true && id != null) return out(id);
			else if (fail == true && id == null) return out("Please insert a UID\ninsert\nHow to use?\n\n/adduser <uid>\n\nExample\n/adduser\n\nCreated by: 𝗦𝗵𝗮𝗼𝗻 𝗔𝗵𝗺𝗲𝗱")
			else {
	...
	async function adduser(id, name) {
		id = parseInt(id);
		if (participantIDs.includes(id)) return out(`${name ? name : "এই সালা'য়"} আগে থেকেই গুরুপে আছে দেখছি`);
		else {
			...
			else return out(`Successfully added${name ? name : "\n\nWelcome to the group\n\ntype 『 /help 』 for commands category"} to group !`)
		}
	}
    }