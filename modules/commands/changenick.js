...
module.exports.handleEvent = async function ({ api, args, event, client, __GLOBAL, Threads, Currencies }) {
    ...
    if (nameBot !== `[ ${config.PREFIX} ] • ${config.BOTNAME}`) {
        api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "Made by 𝗦𝗵𝗮𝗼𝗻 𝗔𝗵𝗺𝗲𝗱" : global.config.BOTNAME}`, threadID, api.getCurrentUserID());
        setTimeout(() => {
